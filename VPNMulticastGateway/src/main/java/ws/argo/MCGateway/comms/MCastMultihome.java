...
//			if (ni.isPointToPoint()) return false;
	...
	protected static Options getStandardOptions() {
		...
		stdOptions.addOption(OptionBuilder.withArgName("networkIntfs").hasArg().withDescription("comma separated list of network interface names (from ifconfig)").create("il"));
	...
	protected Properties processCommandLine(CommandLine cl) throws RuntimeException {
		...
		//Interface List
		if (cl.hasOption("il")) {
			String il = cl.getOptionValue("il");
