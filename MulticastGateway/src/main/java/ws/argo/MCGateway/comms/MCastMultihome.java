...
public abstract class MCastMultihome {
  ...
  /**
   * Print the header for the gateway.
   * 
   * @param p the list of properties to print
   */
   ...
   * @throws SocketException if something goes wrong opening connecting to the
   *           multicast address
   */
  ...
  protected void setCliValues(Properties cliValues) {
    ...
    if (this.mport == null) {
      this.mport = MULTICAST_PORT;
    }
    this.networkIntfs = (List<String>) cliValues.get("il");
    if (this.networkIntfs == null) {
      this.networkIntfs = new ArrayList<String>();
    }
